...
Mechanical cleanup only: each qualifying toggle is treated as permanently `true`. No refactoring, no renames, no behavior changes beyond deleting the dead disabled path. The one allowed — and expected — reshaping is folding away a value the inline has turned into a compile-time constant (a default argument, a stored or computed property, or a struct flag now pinned to a literal). This is a required part of the removal, not optional polish — see the constant-fold bullet under Dead-code chase and the fold step in Finishing.

The bar for deleting anything is that it is provably dead — no remaining references anywhere (`rg` by name), or unreachable after the inline. Code that still compiles and still has a live caller stays exactly as written, even when the removal leaves it looking redundant — a lone `V2` suffix, a one-case `switch`, an indirection with a single implementation. Renaming, collapsing, and re-shaping such code are judgment calls; leave them to the toggle's author, who is added as a PR reviewer (see Finishing) and can clean up as a follow-up. When unsure whether something is dead or merely ugly, leave it — but this "leave it / leave to the author" guidance covers only cosmetic renames and restructures of code that still has live callers. It does NOT cover constant-folds: a value the inline pins to a constant (`someFlag: Bool = true`, a `{ true }` computed property) is provably dead state rather than a matter of taste, and you must fold it away as described below — proactively, in the same pass, without being asked.
...
- a parameter, defaulted argument, or stored property the inline pins to a compile-time constant (e.g. `isRatingFeatureAvailable: Bool = true` once `FeatureProvider.isAvailable(.x)` folded to `true`) — drop it, keep only the branch the constant selects, and update every call site and test. Do this only when `rg` by name confirms no caller passes a different value (everyone relies on the constant default, or every explicit argument is that same literal). It reshapes signatures rather than just deleting a dead branch, so keep it in a separate commit layered on top of the per-toggle removals — but do it, in the same pass, without waiting to be asked; it is required cleanup. Never fold a value that still receives a meaningful, non-constant value from any caller. If a fold's dead-code chase would balloon — deleting a whole flow/subsystem, crossing into another feature's public API, or touching a sensitive domain (payments, crypto, signing) across many files — do the small, contained folds now and, for the large one, STOP: report its scope to the user and offer to split it into its own ticket/PR (treat it like an oversized toggle). Don't silently skip it, and don't unilaterally land a large, risky refactor;
...
2. Fold every degenerate value the inlines created — default arguments, stored/computed properties, and struct flags the inline pinned to a literal. `rg`-verify no caller passes a different value, then drop the value and collapse its readers to the selected branch — one separate commit per fold, layered on the per-toggle removals. Required, not optional (see the constant-fold bullet under Dead-code chase, including the escalation rule for oversized folds).
3. Build the app scheme; fix breakage only by further mechanical deletion.
4. Run the affected unit tests (app target and any touched SPM modules).
5. Format what you changed: `mint run swiftformat@0.55.5 <changed paths> --config .swiftformat`.
6. One commit per toggle keeps the PR reviewable; the heavy cascades (deleted flows) and each fold get their own commit.
7. List in the PR description any localization keys the removal orphaned. A key is orphaned when the legacy `Localization.<accessor>` it backs has no references left outside `Generated/` — find them by mapping each deleted string usage to its accessor and confirming `rg --no-ignore '\.<accessor>\b' --glob '!**/Generated/**'` returns nothing (report the Lokalise key from the accessor's `tr("Localizable", "<key>")`). These can't be removed from here (SwiftGen/Lokalise own them), so the description is where someone marks them deprecated by hand.
8. Request the authors of the removed toggles as PR reviewers. For each toggle, find the commit that introduced it and resolve the author's GitHub login:
...
9. Add a line to the PR description asking each removed toggle's author (the reviewers resolved in step 8) to validate the QA Notes generated for this removal — they own the feature and are best placed to confirm the auto-generated scenarios actually cover it now that the disabled path is gone. Mention them by GitHub login, e.g. `@<login> — please validate the generated QA Notes for the <toggle> removal.`
10. The standard workflow from AGENTS.md applies: Jira ticket fields, branch naming, self-review before the PR.

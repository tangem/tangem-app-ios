...
actor CommonP2PBatchBalancesService: P2PBatchBalancesService {
    ...
    func balances() async throws -> [String: [StakingBalanceInfo]] {
        let addresses = await addressProvider.delegatorAddresses()
        return try await fetch(for: addresses, forceRefresh: false)
    }
    ...
    private func fetchAll(delegatorAddresses addresses: [String]) async throws -> [String: [StakingBalanceInfo]] {
        ...
        let infos: [P2PDTO.AccountsList.AccountsListInfo] = try await TaskGroup.tryExecuteKeepingOrder(items: vaults) { [service] vault in

...
extension ExpressAPITarget: TargetTypeLogConvertible {
    var requestDescription: String {
        guard let body = encodedRequestBody else { return path }
        return "\(path) \(body)"
    }

    var shouldLogResponseBody: Bool { true }

    private var encodedRequestBody: String? {
        let request: (any Encodable)? = switch target {
        case .assets(let request): request
        case .pairs(let request): request
        case .exchangeQuote(let request): request
        case .exchangeData(let request): request
        case .exchangeStatus(let request): request
        case .exchangeSent(let request): request
        case .onrampPairs(let request): request
        case .onrampQuote(let request): request
        case .onrampData(let request): request
        case .onrampNativePaymentData(let request): request
        case .onrampStatus(let request): request
        case .providers,
             .onrampCurrencies,
             .onrampCountries,
             .onrampCountryByIP,

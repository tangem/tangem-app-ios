...
extension CommonExpressManager: ExpressManager {
    ...
    func updateQuotes(
        ...
        guard amountType.amount > 0 else {
            ExpressLogger.warning(self, "Amount is zero. Skipping quote update.")
            return
        }
        ...
        let candidates = providers.filteredByRateType(amountType.rateType)
        let names = candidates.map { $0.provider.name }.joined(separator: ", ")
        ExpressLogger.info(self, "Stateless: updating quotes in providers: \(names)")
        ...
        // Update isBest flags
        let bestRate = bestByRateProvider(from: candidates, amountType: amountType)
        let eligibleCount = eligibleProviders(from: candidates).count
        providers.forEach { provider in
            let isBest = eligibleCount > 1 && provider.provider == bestRate?.provider
    ...
    func selectBest(
        ...
        if candidates.count > 1 {
            if let bestByRate = bestByRateProvider(from: candidates, amountType: amountType) {
                return bestByRate
            }
        }
    ...
    func computeRateType(
        ...
        guard let amountType, let selected = selectedProvider else { return nil }
...
private extension CommonExpressManager {
    ...
    /// Stateless variant: finds best provider by rate using explicit amountType parameter.
    func bestByRateProvider(
        from candidates: [ExpressAvailableProvider],
        amountType: ExpressAmountType?
    ) -> ExpressAvailableProvider? {
        let isFixedRate = amountType?.rateType == .fixed
        let eligible = eligibleProviders(from: candidates)

...
private extension CEXExpressProviderManager {
    func getState(request: ExpressManagerSwappingPairRequest) async -> ExpressProviderManagerState {
        do {
            ...
            let expressFee = ExpressFee(option: request.feeOption, fee: estimatedFee)

            return .preview(.init(provider: provider, expressFee: expressFee, subtractFee: subtractFee, quote: quote))

...
extension BaseWalletManager: WalletUpdater {
...
extension BaseWalletManager: WalletManagerUpdater {
    ...
    private func startUpdating() async {
        do {
            ...
            switch (self, keyType) {
            case (let updater as BaseWalletManagerUpdater, .address(let address)):
                try await updater.updateWalletManager(address: address)

            // Ignore `legacy` address here. XDC, Decimal blockchains
            case (let updater as BaseWalletManagerUpdater, .addresses(let `default`, _)):
                try await updater.updateWalletManager(address: `default`)

            case (let updater as MultiAddressesWalletManagerUpdater, .address(let address)):
                try await updater.updateWalletManager(addresses: [address])

            case (let updater as MultiAddressesWalletManagerUpdater, .addresses(let `default`, let legacy)):
                try await updater.updateWalletManager(addresses: [`default`, legacy])

            case (let updater as XPUBWalletManagerUpdater, .xpub(let xpub)):

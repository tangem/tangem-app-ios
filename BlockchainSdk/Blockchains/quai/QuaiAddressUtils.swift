...
class QuaiAddressDerivation {
    ...
    private static func checkAddressZone(address: String, expectedZone: Zone) -> Bool {
...
class HDNode {
    ...
    init(privateKey: Data, publicKey: Data, chainCode: Data, address: String, index: UInt32) {
    ...
    /// Деривирует дочерний узел
    func deriveChild(index: UInt32) throws -> HDNode {
        // Реализация BIP32 деривации
        // Это упрощенная версия - в реальности нужна полная реализация BIP32
        
        let isHardened = index >= 0x80000000
        
        if isHardened {
            // Hardened derivation: HMAC-SHA512(parentKey + parentChainCode, index)
            let data = privateKey + Data(bytes: &index, count: 4)
            let hmac = HMAC<SHA512>.authenticationCode(for: data, using: SymmetricKey(data: chainCode))
            // ... обработка результата
        } else {
            // Non-hardened derivation: HMAC-SHA512(parentPublicKey + parentChainCode, index)
            let data = publicKey + Data(bytes: &index, count: 4)
            let hmac = HMAC<SHA512>.authenticationCode(for: data, using: SymmetricKey(data: chainCode))
            // ... обработка результата
        }
        
        // Возвращаем новый узел
        // (Это упрощенная версия - нужна полная реализация)
        return HDNode(
            privateKey: Data(),
            publicKey: Data(),
            chainCode: Data(),

...
struct VeChainTransactionTests {
    ...
    func signingCoinTransaction() throws {
        ...
        let value = try #require(Decimal(string: "1.2"))
    ...
    func signingTokenTransaction() throws {
        ...
        let value = try #require(Decimal(string: "2.45"))
    ...
    @Test
    func gasPriceCoefficientMapping() {
        #expect(feeCalculator.gasPriceCoefficient(from: .low) == 0)
        #expect(feeCalculator.gasPriceCoefficient(from: .medium) == 127)
        #expect(feeCalculator.gasPriceCoefficient(from: .high) == 255)
    }

    @Test
    func gasForSingleCoinTransferClause() {
        ...
        #expect(gas == 5000 + 16000)
    }

    @Test
    func gasForMultipleCoinTransferClauses() {
        let clauseRaw = VeChainClause.with { input in
            input.to = "0x207F32eB8d9E6f5178336f86c2ebc3E1A4f87211"
            input.value = Data(hexString: "01619B25004CF40000") // 25.48
            input.data = Data()
        ...
        #expect(gas == 5000 + 16000 * 3)
    }

    @Test
    func gasForSingleTokenTransferClause() {
        ...
        #expect(gas == 5000 + 16000 + 4 * 7 + 68 * 43 + 15000)
    }

    @Test
    func gasForMultipleTokenTransferClauses() {
        // 7 zero bytes, 43 non-zero bytes
        ...
        #expect(gas == 5000 + 16000 * 2 + 4 * 7 + 68 * 43 + 4 * 4 + 68 * 46 + 15000)
    }

    // MARK: - Fees

    @Test
    func feeForCoinTransfers() throws {
        let expectedValues = [
        ...
        try zip(VeChainFeeParams.TransactionPriority.allCases, expectedValues)
            .forEach { priority, expectedValue in
                ...
                let expectedValue = try #require(expectedValue)

...
final class HederaTests {
    ...
    @Test
    func signingCoinTransactionECDSA() throws {
        ...
        let value = try #require(Decimal(string: "7.2"))
        ...
        let fee = Fee(feeAmount, parameters: HederaFeeParams(additionalHBARFee: .zero, erc20TransferConfiguration: nil))
    ...
    @Test
    func signingCoinTransactionEdDSA() throws {
        ...
        let signatures = try hashesToSign.map { digest in
            return try #require(privateKey.sign(digest: digest, curve: curve))
        }
    ...
    @Test
    func signingTokenAssociationECDSA() throws {
        setUp(curve: .secp256k1)
    ...
    @Test
    func signingTokenAssociationEdDSA() throws {
        setUp(curve: .ed25519_slip0010)
    ...
    @Test
    func signingTokenTransactionECDSA() throws {
        setUp(curve: .secp256k1)
        ...
        let value = try #require(Decimal(string: "32.43"))
    ...
    @Test
    func signingTokenTransactionEdDSA() throws {
        setUp(curve: .ed25519_slip0010)
        ...
        let value = try #require(Decimal(string: "15.82"))
        let amount = Amount(with: blockchain, type: .token(value: token), value: value)
    ...
    @Test
    func transactionIdConversionFromConsensusToMirrorPositiveCase() throws {
        ...
        #expect(converted == "0.0.3573746-1714034073-123382080")
    }

    @Test
    func transactionIdConversionFromMirrorToConsensusPositiveCase() throws {
        let converter = HederaTransactionIdConverter()
    ...
    @Test
    func transactionIdConversionFromConsensusToMirrorNegativeCase() throws {
        let converter = HederaTransactionIdConverter()

        #expect(throws: (any Error).self) {
            try converter.convertFromConsensusToMirror("0.0.3573746-1714034073.123382080")
        }
    }

    @Test
    func transactionIdConversionFromMirrorToConsensusNegativeCase() throws {
        let converter = HederaTransactionIdConverter()

        #expect(throws: (any Error).self) {
            try converter.convertFromMirrorToConsensus("0.0.3573746-1714034073.123382080")
        }
    }
    ...
    @Test
    func contractAddressConversionFromEVMToHederaNegativeCase() throws {
        let converter = HederaTokenContractAddressConverter()

        #expect(throws: (any Error).self) {
            try converter.convertFromEVMToHedera("0.0.786931")
        }
        #expect(throws: (any Error).self) {
            try converter.convertFromEVMToHedera("0.786931")
        }
        #expect(throws: (any Error).self) {
            try converter.convertFromEVMToHedera("7677bbb545a") // Invalid EVM address
        }
    }

    @Test
    func contractAddressConversionFromHederaToEVMNegativeCase() throws {
        let converter = HederaTokenContractAddressConverter()
        ...
        #expect(throws: (any Error).self) {
            try converter.convertFromHederaToEVM("7677bbb545a") // Invalid EVM address
        }
    }

...
public struct BadgeShowcase: View {
    @State private var size: BadgeSize = .x6
    @State private var variant: BadgeVariant = .tinted
    @State private var appearance: BadgeAppearance = .neutral
    ...
    private var pickerSection: some View {
        VStack(spacing: 8) {
            Picker("Size", selection: $size) {
                Text("x9").tag(BadgeSize.x9)
                Text("x6").tag(BadgeSize.x6)
                Text("x4").tag(BadgeSize.x4)
            }
            .pickerStyle(.segmented)

            Picker("Variant", selection: $variant) {
                Text("tinted").tag(BadgeVariant.tinted)
                Text("solid").tag(BadgeVariant.solid)
                Text("outline").tag(BadgeVariant.outline)
            }
            .pickerStyle(.segmented)

            Picker("Appearance", selection: $appearance) {
                Text("neutral").tag(BadgeAppearance.neutral)
                Text("info").tag(BadgeAppearance.info)
                Text("error").tag(BadgeAppearance.error)
                Text("warning").tag(BadgeAppearance.warning)
                Text("success").tag(BadgeAppearance.success)
            }
            .pickerStyle(.segmented)
    ...
    private var configuredBadge: some View {
        let base = Badge(label: customText, accessibilityLabel: nil)
            .size(size)
            .variant(variant)
...
private struct BadgeMatrixView: View {
    let variants: [BadgeVariant] = [.tinted, .solid, .outline]
    let appearances: [BadgeAppearance] = [.neutral, .info, .error, .warning, .success]
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 8) {
                        ForEach(appearances, id: \.self) { appearance in
                            Badge(label: appearanceName(appearance), accessibilityLabel: nil)
                                .variant(variant)
                                .appearance(appearance)
                        }
                    }
    ...
    private func variantName(_ variant: BadgeVariant) -> String {
        switch variant {
        case .tinted: "Tinted"
        case .solid: "Solid"
    ...
    private func appearanceName(_ appearance: BadgeAppearance) -> String {
        switch appearance {
...
private struct BadgeSizeComparisonView: View {
    ...
    private func sizeName(_ size: BadgeSize) -> String {
        switch size {
        case .x9: "x9"
        case .x6: "x6"
        case .x4: "x4"

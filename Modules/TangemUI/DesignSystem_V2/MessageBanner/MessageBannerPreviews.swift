...
public struct MessageBannerShowcase: View {
    ...
    @State private var variant: MessageBannerVariant = .default
    @State private var contentAlign: MessageBannerContentAlign = .start
    ...
    private var previewBanner: some View {
        baseBanner
            ...
            .slotEnd {
                if hasCloseButton {
                    MessageBannerCloseButton(accessibilityLabel: "Dismiss") {}
                } else if hasSlotEnd {
                    circlePlaceholder
                }
            }
    ...
    private var baseBanner: MessageBanner<EmptyView, EmptyView, EmptyView> {
        ...
        if customText {
            return MessageBanner(title: customTitle, description: description.map { AttributedString($0) })
        }

        return MessageBanner(title: titleInput, description: description)
    }
    ...
    private var controls: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            pickerRow(title: "Variant", cases: MessageBannerVariant.allCases, binding: $variant) {
                String(describing: $0)
            }
            .pickerStyle(.menu)

            pickerRow(title: "Content align", cases: MessageBannerContentAlign.allCases, binding: $contentAlign) {
                String(describing: $0)
            }
    ...
    ScrollView {
        VStack(spacing: 16) {
            ForEach(MessageBannerVariant.allCases, id: \.self) { variant in
                MessageBanner(title: String(describing: variant), description: "Description")
                    .variant(variant)
...
#Preview("Content align") {
    VStack(spacing: 16) {
        ForEach(MessageBannerContentAlign.allCases, id: \.self) { align in
            MessageBanner(title: "Content align \(String(describing: align))", description: "Share Tangem, give 10% OFF, and earn 10 USDT.")
                .contentAlign(align)
...
#Preview("Slots & close") {
    VStack(spacing: 16) {
        MessageBanner(title: "Title", description: "Description")
            .closeButton(accessibilityLabel: "Dismiss") {}
            .secondaryButton(.init(title: "Label", action: {}))
            .primaryButton(.init(title: "Label", action: {}))

        MessageBanner(title: "Invite friends. Earn 10 USDT.", description: "Share Tangem, give 10% OFF, and earn 10 USDT.")
            .slotStart {
                DesignSystem.Icons.Bell.regular24.image
                    .renderingMode(.template)
                    .resizable()

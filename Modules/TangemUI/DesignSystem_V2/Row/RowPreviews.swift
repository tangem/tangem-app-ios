...
public struct RowShowcase: View {
    @State private var contentLead: RowContentLead = .equal
    @State private var verticalAlignment: RowVerticalAlignment = .top
    ...
    @State private var lineOrder: RowLineOrder = .primaryFirst
    ...
    private var controls: some View {
        group("Layout", initiallyExpanded: true) {
            pickerRow(
                title: "contentLead",
                cases: RowContentLead.allCases,
                binding: $contentLead,
                label: { String(describing: $0) }
            )
            pickerRow(
                title: "verticalAlignment",
                cases: RowVerticalAlignment.allCases,
    ...
    private var rowPreview: some View {
        ...
        .titleAccessory {
            if showTitleAccessory {
                Badge(label: titleAccessorySample, accessibilityLabel: nil)
            }
        }
        .subtitleAccessory {
            if showSubtitleAccessory {
                Badge(label: subtitleAccessorySample, accessibilityLabel: nil)
                    .size(.x4)
            }
        }
        .valueAccessory {
            if showValueAccessory {
                Badge(label: valueAccessorySample, accessibilityLabel: nil)
                    .appearance(.success)
            }
        }
        .subvalueAccessory {
            if showSubvalueAccessory {
                Badge(label: subvalueAccessorySample, accessibilityLabel: nil)
                    .size(.x4)
            }
        }
...
#if DEBUG
private func tangemRowVAlignSample(_ alignment: RowVerticalAlignment, extra: Bool) -> some View {
    Row(title: "Title", subtitle: "Subtitle", value: "Value", subvalue: "Subvalue")
        .titleAccessory { Badge(label: "New", accessibilityLabel: nil) }
        .valueAccessory { Badge(label: "+2.3%", accessibilityLabel: nil).appearance(.success) }

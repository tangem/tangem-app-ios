...
public struct LoaderShowcase: View {
    @State private var size: Loader.Size = .size24
    ...
    private var pickerSection: some View {
        VStack(spacing: 8) {
            Picker("Size", selection: $size) {
                Text("12").tag(Loader.Size.size12)
                Text("16").tag(Loader.Size.size16)
                Text("20").tag(Loader.Size.size20)
                Text("24").tag(Loader.Size.size24)
                Text("28").tag(Loader.Size.size28)
                Text("32").tag(Loader.Size.size32)
            }
...
private struct LoaderSizeGalleryView: View {
    ...
    private func row(label: String, size: Loader.Size) -> some View {
        HStack(spacing: 16) {
            Text(label)

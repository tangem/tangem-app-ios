...
public struct CheckboxShowcase: View {
    @State private var value: Checkbox.Value = .unchecked
    ...
    private var controls: some View {
        VStack(spacing: 16) {
            Picker("Value", selection: $value) {
                Text("Unchecked").tag(Checkbox.Value.unchecked)
                Text("Checked").tag(Checkbox.Value.checked)
                Text("Indeterminate").tag(Checkbox.Value.indeterminate)
            }
        ...
        VStack(alignment: .leading, spacing: 24) {
            ForEach([true, false], id: \.self) { enabled in
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 24) {
                        ForEach(Checkbox.Value.allCases, id: \.self) { value in
                            Checkbox(value: value) {}

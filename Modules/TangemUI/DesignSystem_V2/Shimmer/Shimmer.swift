...
/// ```swift
/// Shimmer()
/// Shimmer().variant(.text(style: .body))                          // bar fills the style's default share of the parent
/// Shimmer().variant(.text(style: .body, alignment: .center))
/// Shimmer().variant(.custom(width: 100, height: 24))
/// ```
public struct Shimmer: View {
    ...
    public var body: some View {
        ...
        case .text(let style, let alignment):
            ShimmerTextBlock(style: style, alignment: alignment)

        case .custom(let width, let height, let cornerRadius):
            ShimmerCustomBlock(width: width, height: height, cornerRadius: cornerRadius)
        }
    }
}
...
private struct ShimmerTextBlock: View {
    let style: Shimmer.TextStyle
    let alignment: Shimmer.Alignment
    ...
    init(style: Shimmer.TextStyle, alignment: Shimmer.Alignment) {
        self.style = style
...
private struct ShimmerCustomBlock: View {
    let width: CGFloat?
    let height: CGFloat?

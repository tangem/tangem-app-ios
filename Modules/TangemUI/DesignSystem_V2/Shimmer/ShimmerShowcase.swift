...
public struct ShimmerShowcase: View {
    ...
    @State private var textAlignment: Shimmer.Alignment = .leading
        ...
        section(title: "Text variant — every style") {
            VStack(alignment: .leading, spacing: 12) {
                Picker("Alignment", selection: $textAlignment) {
                    Text("Leading").tag(Shimmer.Alignment.leading)
                    Text("Center").tag(Shimmer.Alignment.center)
                    Text("Trailing").tag(Shimmer.Alignment.trailing)
                }
                .pickerStyle(.segmented)

                ForEach(Shimmer.TextStyle.allCases, id: \.self) { style in
                    HStack(spacing: 12) {
                        Text(String(describing: style))
                            .font(.caption.monospaced())
                            .foregroundStyle(.secondary)
                            .frame(width: 110, alignment: .leading)
                        Shimmer().variant(.text(style: style, alignment: textAlignment))
                    }
                }
            }
        }
            ...
            VStack(spacing: 12) {
                ForEach(0 ..< 3) { _ in
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Shimmer().variant(.text(style: .body))
                            Shimmer().variant(.text(style: .caption))
                        }
                    }
                }
            }
    ...
    private var screenExampleSection: some View {
        section(title: "Screen skeleton — composed primitives") {
            VStack(alignment: .leading, spacing: 16) {
                Shimmer().variant(.text(style: .display))
                Shimmer().variant(.text(style: .subheading))
                Spacer().frame(height: 8)
                Shimmer().variant(.custom(height: 120, cornerRadius: 16))
                HStack(spacing: 12) {
                    Shimmer().variant(.custom(height: 56, cornerRadius: 16))
                    Shimmer().variant(.custom(height: 56, cornerRadius: 16))
                }
            }
            .padding(16)

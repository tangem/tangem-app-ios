...
public struct FadeShowcase: View {
    @State private var variant: Fade.Variant = .soft
    @State private var position: Fade.Position = .bottom
        ...
        ScrollView {
            VStack(spacing: 12) {
                Picker("Variant", selection: $variant) {
                    Text("soft").tag(Fade.Variant.soft)
                    Text("hard").tag(Fade.Variant.hard)
                }
                .pickerStyle(.segmented)

                Picker("Position", selection: $position) {
                    Text("top").tag(Fade.Position.top)
                    Text("bottom").tag(Fade.Position.bottom)
                }
                .pickerStyle(.segmented)
...
private extension Fade.Position {
...
private struct FadeGalleryView: View {
    ...
    private func row(title: String, variant: Fade.Variant, position: Fade.Position) -> some View {

...
private enum MacroError: DiagnosticMessage {
    case notAnEnum

    var message: String {
        switch self {
        case .notAnEnum:
            return "The '@CaseFlagableMacro' macro can only be applied to enums."
        }
    }

    var diagnosticID: MessageID { MessageID(domain: "CaseFlagableMacro", id: "Error") }

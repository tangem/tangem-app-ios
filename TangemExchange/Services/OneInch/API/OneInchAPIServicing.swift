...
public protocol OneInchAPIServicing {
    ...
    ) async -> Result<QuoteData, ExchangeProviderError>

    // Generating data for exchange
    func swap(
        blockchain: ExchangeBlockchain,
        parameters: ExchangeParameters
    ) async -> Result<ExchangeData, ExchangeProviderError>

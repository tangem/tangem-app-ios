...
public class UtxoSelector: UtxoSelectorInterface {
    ...
    public func select(from utxos: [HDUnspentTransaction], targetValue: UInt64) throws -> (utxos: [HDUnspentTransaction], fee: UInt64) {
        ...
        let sortedUtxos: [HDUnspentTransaction] = utxos.sorted(by: { $0.output.value < $1.output.value })
...
internal extension Sequence where Element == HDUnspentTransaction {
    func sum() -> UInt64 {

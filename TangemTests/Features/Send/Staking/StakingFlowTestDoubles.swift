...
final class SendTransactionValidatorMock: SendTransactionValidator {
    ...
    /// When set, mimics `TransactionValidator.validateTotal`: amount+fee must fit into the balance.
    var balance: Decimal?

    init(balance: Decimal? = nil) {
    ...
    func validate(amount: Amount, fee: Fee) throws {
        ...
        if let balance, amount.value + fee.amount.value > balance {
...
final class StakingPreflightValidatorMock: StakingPreflightValidator {
    var failure: StakingPreflightFailure?
    private(set) var validateCallsCount = 0

    init(failure: StakingPreflightFailure? = nil) {
...
// MARK: - StakingValidationProvider

final class StakingValidationProviderMock: StakingValidationProvider {
    var result: StakingValidationResult

    init(state: StakingValidationState = .validated, transaction: StakingTransactionAction? = nil) {
        result = StakingValidationResult(state: state, transaction: transaction)
    }

//
//  StakeStagesResolverTests.swift
//  TangemTests
//
...
@Suite("StakeStagesResolver")
struct StakeStagesResolverTests {
    // MARK: - finalize

    @Test("Finalize returns ready without reducing the amount when the fee is not included")
    func finalizeReady() {
        guard case .ready(let ready) = makeStages().finalize(amount: 10, fee: 1, target: nil, isAmountEditable: true, includesStakesCount: false, isEnter: true, enterSpendsAmount: true) else {
    ...
    func finalizeFeeIncluded() {
        ...
        guard case .ready(let ready) = stages.finalize(amount: 10, fee: 1, target: nil, isAmountEditable: true, includesStakesCount: false, isEnter: true, enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }
    ...
    func finalizeFixedIgnoresFeeInclusion() {
        ...
        guard case .ready(let ready) = stages.finalize(amount: 10, fee: 1, target: nil, isAmountEditable: false, includesStakesCount: false, isEnter: true, enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }
        #expect(ready.amount == 10)
    ...
    func finalizeValidationError() {
        ...
        guard case .failure(.transaction) = stages.finalize(amount: 10, fee: 1, target: nil, isAmountEditable: true, includesStakesCount: false, isEnter: true, enterSpendsAmount: true) else {
    ...
    func finalizeNonEnterKeepsAmount() {
        ...
        guard case .ready(let ready) = stages.finalize(amount: 10, fee: 1, target: nil, isAmountEditable: true, includesStakesCount: false, isEnter: false, enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }
        #expect(ready.amount == 10)
    ...
    @Test("An enter whose amount stays in the wallet validates the fee alone against the balance")
    func finalizeEnterThatDoesNotSpendAmount() {
        // The whole balance is staked, so validating amount+fee would always exceed it.
        let stages = makeStages(transactionValidator: SendTransactionValidatorMock(balance: 10))

        guard case .ready(let ready) = stages.finalize(amount: 10, fee: 1, target: nil, isAmountEditable: false, includesStakesCount: false, isEnter: true, enterSpendsAmount: false) else {
            Issue.record("Expected ready")
            return
        }
        #expect(ready.amount == 10)
    ...
    @Test("An enter that spends the amount still has to clear the balance together with the fee")
    ...
    func resolveCommonReady() async throws {
        ...
        guard case .ready = try await stages.resolveCommon(action: stakeAction(amount: 10), stepPlan: editableStepPlan(), enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }
    }
    ...
    func resolveCommonAccountInit() async throws {
        ...
        guard case .prerequisite(.accountInitialization(.required)) = try await stages.resolveCommon(action: stakeAction(amount: 10), stepPlan: editableStepPlan(), enterSpendsAmount: true) else {
    ...
    func resolveCommonPropagatesEstimateError() async throws {
        ...
        await #expect(throws: StakingManagerMockError.self) {
            _ = try await stages.resolveCommon(action: stakeAction(amount: 10), stepPlan: editableStepPlan(), enterSpendsAmount: true)
        }
    }
    ...
    func enterExtrasAmountToReduce() async throws {
        ...
        guard case .ready(let ready) = try await stages.resolveCommon(action: stakeAction(amount: 10), stepPlan: editableStepPlan(), enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }
    ...
    func nonEnterReadyHasNoExtras() async throws {
        ...
        guard case .ready(let ready) = try await stages.resolveCommon(action: unstake, stepPlan: editableStepPlan(), enterSpendsAmount: true) else {
            Issue.record("Expected ready")
            return
        }

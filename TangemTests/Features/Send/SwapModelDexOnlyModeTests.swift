...
struct SwapModelDexOnlyModeTests {
    ...
    func hideProvidersKeepsLegacyEmptyState() async throws {
        ...
        #expect(await environment.expressManager.updateAmountCallsCount == 0)
    }
    ...
    @Test("DEX-only mode falls back to the legacy empty state when no provider is selected")
    func dexOnlyModeFallsBackWhenNoProviderSelected() async throws {
        let environment = makeEnvironment(restriction: .dexProvidersOnly)
        let dex = makeProvider(id: "dex", type: .dex, expectAmount: 90)
        // The engine produced no eligible selection (e.g. every quote is below the minimum),
        // so even though a DEX exists in the set there is nothing usable to show
        await environment.expressManager.configure(state: .swap(selected: nil, providers: makeProviders(dex)))

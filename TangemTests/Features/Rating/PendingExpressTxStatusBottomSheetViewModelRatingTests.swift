...
final class PendingExpressTxStatusBottomSheetViewModelRatingTests {
    ...
    func ratingViewModelCreatedOnUpdate() async {
        let (sut, subject, manager) = makeSUT(externalTxId: nil)
    ...
    func ratingViewModelCreatedOnlyOnce() async {
        let (sut, subject, manager) = makeSUT(externalTxId: anyExternalID)

        let firstInstance = sut.ratingViewModel
    ...
    func ratingViewModelPublishesChanges() async {
        let (sut, subject, manager) = makeSUT(externalTxId: nil)
...
private extension PendingExpressTxStatusBottomSheetViewModelRatingTests {
    func setupRatingFeature() {
        FeatureStorage.instance.availableFeatures[.swapRateExperience] = .on
    ...
    ) -> (sut: SUT, subject: CurrentValueSubject<[PendingTransaction], Never>, manager: StubPendingExpressTransactionsManager) {
...
private struct StubKeysManager: KeysManager {
    var surveySparrow: SurveySparrowKeys {
        // Decode from JSON since SurveySparrowKeys only has Decodable init
        let json = """
        {

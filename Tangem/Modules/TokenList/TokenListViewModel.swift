    ...
    func update(cardModel: CardViewModel, entries: inout [StorageEntry]) {
        pendingRemove.forEach { tokenItem in
            ...
            case let .blockchain(blockchain):
                entries.removeAll { $0.blockchainNetwork.blockchain.storageId == blockchain.storageId }
            case let .token(token, blockchain):
                if let index = entries.firstIndex(where: { $0.blockchainNetwork.blockchain.storageId == blockchain.storageId }) {

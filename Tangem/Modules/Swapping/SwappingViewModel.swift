...
private extension SwappingViewModel {
    ...
    func openPermissionView() {
        ...
        guard case .available(let result, let info) = state, result.isPermissionRequired else {
            return
        }

        Task {
            let source = exchangeManager.getExchangeItems().source
            let fiatFee = try await fiatRatesProvider.getFiat(for: source, amount: info.fee)

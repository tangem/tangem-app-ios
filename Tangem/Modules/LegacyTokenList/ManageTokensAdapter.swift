...
class ManageTokensAdapter: ObservableObject {
    ...
    private let coinViewModelsSubject = CurrentValueSubject<[LegacyCoinViewModel], Never>([])
    ...
    var coinViewModelsPublisher: some Publisher<[LegacyCoinViewModel], Never> {
    ...
    init(longHashesSupported: Bool, existingCurves: [EllipticCurve], supportedBlockchains: Set<Blockchain>, userTokensManager: UserTokensManager) {
        self.longHashesSupported = longHashesSupported
        self.existingCurves = existingCurves
        self.userTokensManager = userTokensManager
        loader = ListDataLoader(supportedBlockchains: supportedBlockchains)
...
private extension ManageTokensAdapter {
    func bind() {
        loader.$items
            ...
            .map { adapter, items -> [LegacyCoinViewModel] in

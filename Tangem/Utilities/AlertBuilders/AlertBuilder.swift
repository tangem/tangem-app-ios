...
enum AlertBuilder {
    ...
    static func makeActivatedCardAlertController(okAction: @escaping (() -> Void), supportAction: @escaping (() -> Void), cancelAction: @escaping (() -> Void)) -> UIAlertController {
        ...
        alert.addAction(UIAlertAction(title: Localization.alertButtonRequestSupport, style: .default, handler: { _ in supportAction() }))
    ...
    static func makeExitAlert(
        ...
        primaryButtonText: String = Localization.commonNo,
        secondaryButtonText: String = Localization.commonYes,
        okAction: @escaping (() -> Void) = {}
    ) -> AlertBinder {
        .init(alert: Alert(
            title: Text(title),
            message: Text(message),
            primaryButton: .default(Text(primaryButtonText), action: {}),
            secondaryButton: .destructive(Text(secondaryButtonText), action: okAction)
        ))
    }

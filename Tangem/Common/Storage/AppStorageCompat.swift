...
extension AppStorageCompat where Value == Bool {
    ...
    init(wrappedValue: Value, _ key: StorageType, store: UserDefaults? = .init(suiteName: AppEnvironment.suiteName)) {
...
extension AppStorageCompat where Value: RawRepresentable, Value.RawValue == Int {
    ...
    ///   - store: The user defaults store to read and write to. A value
    ///     of `nil` will use the user default store from the environment.
    init(wrappedValue: Value, _ key: StorageType, store: UserDefaults? = nil) {
        let store = (store ?? .standard)

...
class CardViewModel: Identifiable, ObservableObject {
    ...
    func changeSecurityOption(_ option: SecurityModeOption, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        case .accessCode:
            tangemSdk.startSession(with: SetUserCodeCommand(accessCode: nil),
                                   cardId: cardId,
                                   initialMessage: Message(header: nil, body: L10n.initialMessageChangeAccessCodeBody)) { [weak self] result in
        ...
        case .passCode:
            tangemSdk.startSession(with: SetUserCodeCommand(passcode: nil),
                                   cardId: cardId,
                                   initialMessage: Message(header: nil, body: L10n.initialMessageChangePasscodeBody)) { [weak self] result in
                guard let self = self else { return }
    ...
    func resetToFactory(completion: @escaping (Result<Void, TangemSdkError>) -> Void) {
        ...
        tangemSdk.startSession(with: ResetToFactorySettingsTask(),
                               cardId: cardId,
                               initialMessage: Message(header: nil,
                                                       body: L10n.initialMessagePurgeWalletBody)) { [weak self] result in
            switch result {
            case .success:

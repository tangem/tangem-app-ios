...
//    }
        ...
        searchBlockchainsCancellable =
            ...
            .sink(receiveValue: { [unowned self] _ in
                ...
                if !notEmptyWallets.isEmpty {
                    tokenItemsRepository.append(notEmptyWallets.map({TokenItem($0.wallet.blockchain)}), for: cardInfo.card.cardId)
        ...
        tokenFinder.findErc20Tokens(knownTokens: knownTokens) {[weak self] result in
            ...
            case .success(let tokensAdded):
                if tokensAdded {
                    ...
                    let tokenItems = tokens.map { TokenItem($0) }
                    self.tokenItemsRepository.append(tokenItems, for: self.cardInfo.card.cardId)
                    
                    if sholdAddWalletManager {
                        self.tokenItemsRepository.append(.init(ethWalletModel!.wallet.blockchain), for: self.cardInfo.card.cardId)
    ...
    private func removeBlockchain(_ blockchain: Blockchain) {
        tokenItemsRepository.remove(.init(blockchain), for: cardInfo.card.cardId)

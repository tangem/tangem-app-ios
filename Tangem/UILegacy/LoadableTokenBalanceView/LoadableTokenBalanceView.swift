...
// MARK: - Yield Identification Modifier

private struct YieldIdentificationModifier: ViewModifier {
    let isEnabled: Bool
    let state: LoadableTokenBalanceView.State
    let showInfoAction: (() -> Void)?

    func body(content: Content) -> some View {
        if isEnabled, case .loaded = state {
            HStack(spacing: 4) {
                exchangeIcon
                content
                infoButton
...
extension LoadableTokenBalanceView {
    func yieldIdentificationIfNeeded(_ isEnabled: Bool, showInfoAction: (() -> Void)?) -> some View {
        modifier(YieldIdentificationModifier(isEnabled: isEnabled, state: state, showInfoAction: showInfoAction))
    }
}

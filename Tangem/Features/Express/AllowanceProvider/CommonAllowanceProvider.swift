...
extension CommonAllowanceProvider: AllowanceProvider {
    ...
    func allowanceState(amount: Decimal, spender: String, approvePolicy: ApprovePolicy) async throws -> AllowanceState {
        guard let allowanceChecker else {
            throw AllowanceProviderError.allowanceCheckerNotFound
        }
...
enum AllowanceProviderError: String, Hashable, LocalizedError {
    case allowanceCheckerNotFound

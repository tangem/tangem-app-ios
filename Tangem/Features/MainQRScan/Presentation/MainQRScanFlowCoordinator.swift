...
final class MainQRScanFlowCoordinator: CoordinatorObject {
    ...
    private func handleScannedCode(_ code: String) {
        ...
        Task { [weak self] in
            let action = await Self.resolveInBackground(
                code: code,
                blockchains: context.allBlockchains,
                tokenItems: context.allTokenItems,
                resolver: resolver
            )
    ...
    private nonisolated static func resolveInBackground(
        code: String,
        blockchains: [Blockchain],
        tokenItems: [TokenItem],
        resolver: MainQRScanRouteResolver
    ) async -> MainQRScanAction {
        resolver.resolve(
            scannedCode: code,
            availableBlockchains: blockchains,
            availableTokenItems: tokenItems
        )
    }

    @MainActor
    private func route(_ action: MainQRScanAction, allMatches: [WalletModelMatch]) {
        switch action {
    ...
    @MainActor
    private func openSend(with match: WalletModelMatch, parameters: PredefinedSendParameters) {
        closeScanner()
    ...
    private func showNoSupportedTokensAlert(context: MainQRNoSupportedTokensContext? = nil) {
        turnOffScannerFlashIfNeeded()
        let message: String
        if let symbol = context?.symbol, let networkId = context?.networkId {
            // [REDACTED_TODO_COMMENT]
            message = "Token \(symbol) on \(networkId) isn't added to your wallet."
        } else {
            message = Localization.qrScannerErrorUnsupportedNetworkMessage
        }
...
extension MainQRScanFlowCoordinator: MainQRScanTokenSelectorRoutable {
    func didSelectToken(
        ...
        openSend(
            with: WalletModelMatch(walletModel: walletModel, userWalletInfo: userWalletInfo),

...
extension AccountsAwareActionButtonsSellViewModel: AccountsAwareTokenSelectorViewModelOutput {
    func userDidSelect(item: AccountsAwareTokenSelectorItem) {
        guard case .crypto(_, let walletModel) = item.source else { return }

        ActionButtonsAnalyticsService.trackTokenClicked(.sell, tokenSymbol: item.tokenItem.currencySymbol)

//
//  NewActionButtonsSwapView.swift
//  TangemApp
//
...
struct NewActionButtonsSwapView: View {
    @ObservedObject var viewModel: NewActionButtonsSwapViewModel
    ...
    private var header: some View {
        GroupedSection(viewModel.source) { destinationType in
            ...
            case .token(_, let viewModel):
                NewTokenSelectorItemView(viewModel: viewModel)
                    .allowsHitTesting(false)
            }
        } header: {
            NewActionButtonsSwapHeaderView(

...
class MobileOnboardingFlowBuilder: ObservableObject {
    typealias Step = MobileOnboardingFlowStep
...
private extension MobileOnboardingFlowBuilder {
    func startFlow() {
...
extension MobileOnboardingFlowBuilder {
    func back() {
        guard
            let currentNode,
...
extension MobileOnboardingFlowBuilder {
    ...
    var navBarBackAction: MobileOnboardingFlowNavBarAction {
        MobileOnboardingFlowNavBarAction.back(handler: { [weak self] in
            self?.back()
        })
    }
}

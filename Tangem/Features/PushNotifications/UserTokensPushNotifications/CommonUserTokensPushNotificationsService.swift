...
final class CommonUserTokensPushNotificationsService: NSObject {
    ...
    private let applicationClient = UserWalletsPushApplicationClient()
    private let stateSyncer = UserWalletsPushStateSyncer()
    private let walletNameObserver = UserWalletsPushWalletNameObserver()
    ...
    private func updateState() {
        ...
        updateStateTask = runTask(in: self) { service in
            do {
                try await service.stateSyncer.syncUserWalletModelState(applicationUid: service.applicationUid)
                await service.walletNameObserver.restartObserving()
            } catch {
                AppLogger.info("Failed sync remote with remote wallets state")
                service.stateSyncer.handleSyncErrorForAllWallets()
            }
        }
    }
}

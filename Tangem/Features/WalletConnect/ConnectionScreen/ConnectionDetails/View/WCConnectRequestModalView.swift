...
struct WCConnectRequestModalView: View {
    ...
    private var content: some View {
        ...
        .background {
            if viewModel.presentationState != .walletSelector {
    ...
    private var walletSelector: some View {
        WCWalletSelectorView(
            ...
            closeAction: { viewModel.handleViewAction(.returnToConnectionDetails) }
        )
...
private extension WCConnectRequestModalView {
    var selectedWallet: some View {
        HStack(spacing: 0) {
            ...
            if viewModel.canSelectOtherWallet {
                Assets.WalletConnect.selectIcon.image
            }
        }
        .onTapGesture {
            guard viewModel.canSelectOtherWallet else { return }

...
final class WCServiceV2 {
    ...
    private var walletModelsCancellable: AnyCancellable?
...
private extension WCServiceV2 {
        ...
        userWalletRepository
            ...
            .sink { wcService, event in
                ...
                case .deleted(let userWalletIds):
                    ...
                    wcService.walletModelsCancellable?.cancel()
                ...
                case .inserted(let userWalletId):
                    if wcService.userWalletRepository.selectedModel?.userWalletId == userWalletId {
                        wcService.subscribeToWalletModels(for: userWalletId)
                    }

                case .unlocked:
                    if let selectedId = wcService.userWalletRepository.selectedModel?.userWalletId {
                        wcService.subscribeToWalletModels(for: selectedId)
                    }
    ...
    func subscribeToWalletModelsIfNeeded() {
        guard let selectedId = userWalletRepository.selectedModel?.userWalletId else {
            return
        }

        subscribeToWalletModels(for: selectedId)
    }
    ...
    func subscribeToWalletModels(for userWalletId: UserWalletId) {
        walletModelsCancellable?.cancel()

        guard let userWalletModel = userWalletRepository.models[userWalletId] else {
            return
        }

        walletModelsCancellable = AccountsFeatureAwareWalletModelsResolver

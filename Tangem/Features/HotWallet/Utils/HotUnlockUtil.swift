...
extension HotUnlockUtil {
    func unlock() async throws -> Result {
        let manager = await CommonHotAccessCodeManager(
            userWalletId: userWalletId,
            configuration: .default,
            storageManager: CommonHotAccessCodeStorageManager()
        )

        let viewModel = HotUnlockViewModel(userWalletId: userWalletId, accessCodeManager: manager)
        let view = HotUnlockView(viewModel: viewModel)
...
private extension HotUnlockUtil {
    func handleAction(_ action: HotUnlockViewModel.Action) async -> Result? {
        switch action {

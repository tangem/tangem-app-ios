...
final class AddCustomTokenDerivationPathSelectorViewModel: ObservableObject {
    ...
    private weak var coordinator: AddCustomTokenDerivationPathSelectorRoutable?
    ...
    init(
        ...
        let customDerivationOption: AddCustomTokenDerivationOption = if case .custom = selectedDerivationOption {
            selectedDerivationOption
        } else {
            .custom(derivationPath: nil)
        }

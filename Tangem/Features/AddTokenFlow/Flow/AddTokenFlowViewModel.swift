...
@MainActor
final class AddTokenFlowViewModel: ObservableObject, FloatingSheetContentViewModel {
    ...
    private weak var coordinator: AddTokenFlowRoutable?
    ...
    init(
        ...
        coordinator: AddTokenFlowRoutable?
    ) {
...
private extension AddTokenFlowViewModel {
...
extension AddTokenFlowViewModel {
    func close() {
        coordinator?.close()
    }
...
private extension AddTokenFlowViewModel {
    ...
    func openGetToken(
        tokenItem: TokenItem,
        accountSelectorCell: AccountSelectorCellModel,
        config: AddTokenFlowConfiguration.GetTokenConfiguration
    ) {
...
private extension AddTokenFlowViewModel {
    ...
    func makeNetworkDataProvider(
        accountSelectorCell: AccountSelectorCellModel,
        tokenItem: TokenItem
    ) -> AddTokenNetworkSelectorDataProvider {
        let availableTokenItems = configuration.getAvailableTokenItems(accountSelectorCell)
        let isSelectionAvailable = availableTokenItems.count > 1

        return AddTokenNetworkDataProvider(
            tokenItem: tokenItem,
            isSelectionAvailable: isSelectionAvailable,
            handleSelection: { [weak self] in
...
private extension AddTokenFlowViewModel {
    ...
    func makeCryptoAccountModelsFilter() -> (AccountSelectorViewModel.AccountSelectorItem) -> Bool {
        ...
        return { [weak self] accountSelectorItem in
            ...
            let context = AddTokenFlowConfiguration.AccountContext(

...
struct AddTokenFlowView: View {
    @ObservedObject var viewModel: AddTokenFlowViewModel
    ...
    private var contentView: some View {
        ZStack {
            ...
            case .networkSelector(let networkSelectorViewModel, _):
                NetworkSelectorView(viewModel: networkSelectorViewModel)
                    .transition(.content)

            case .addToken(let addTokenViewModel):
                AddTokenView(viewModel: addTokenViewModel)
                    .transition(.content)

            case .getToken(let getTokenViewModel):
                GetTokenView(viewModel: getTokenViewModel)
                    .transition(.content)
            }
        }
    ...
    private func header(from viewState: AddTokenFlowViewModel.ViewState) -> some View {
        let title: String
...
extension AddTokenFlowView {
    private enum Layout {
        static let scrollViewCoordinateSpace = "AddTokenFlowView.ScrollView"
    }
}

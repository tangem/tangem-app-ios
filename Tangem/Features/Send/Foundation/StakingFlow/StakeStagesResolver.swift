...
struct StakeStagesResolver {
    ...
    func buildTransaction(action: StakingAction) async throws -> StakingTransactionAction {
        try await stakingManager.transaction(action: action)
    }
    ...
    func resolveCommon(action: StakingAction, stepPlan: StakeStepPlan, enterSpendsAmount: Bool) async throws -> StakeFlowState {
    ...
    func finalize(
        ...
        includesStakesCount: Bool,
        isEnter: Bool,
        enterSpendsAmount: Bool
    ) -> StakeFlowState {
        let includeFee = isAmountEditable && isEnter
            && feeIncludedCalculator.shouldIncludeFee(makeFee(fee), into: makeAmount(amount))
        let resolvedAmount = includeFee ? amount - fee : amount

        // Only an amount that really leaves the wallet has to clear the balance together with the fee.
        // Where the stake keeps the balance in place (Cardano delegates it, spending fee and deposit only)
        // the flow stakes the whole balance, so validating that amount would always exceed it.
        let spentAmount = isEnter && enterSpendsAmount ? resolvedAmount : Decimal.zero

        if let validationError = validate(amount: spentAmount, fee: fee) {

...
enum StakingFlowProviderFactory {
    static func make(
        ...
        switch network {
        case .solana:
            return SolanaStakingFlowProvider(action: action, stages: stages, preflightValidator: preflightValidator)
        case .cosmos:
            return CosmosStakingFlowProvider(action: action, stages: stages)
        case .kava, .near, .polkadot:
            return UnsupportedStakingFlowProvider(action: action)
        case .bsc:

...
extension SwapModel {
    ...
    func swappingPairDidChange() {
        updateTask { [weak self] expressManager in
    ...
    func updateTask(block: @escaping (_ manager: ExpressManager) async throws -> ExpressManagerUpdatingResult?) {
        updateTask?.cancel()

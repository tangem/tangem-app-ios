...
class StakingMinimumAmountValidator {
    ...
    private let minimumAmount = OSAllocatedUnfairLock<Decimal?>(initialState: nil)
    ...
    private func bind() {
        stakingManagerStatePublisher
            ...
            .sink(receiveValue: { [weak self] amount in
                self?.minimumAmount.withLock { $0 = amount }
            })
            .store(in: &bag)
    }
}
...
extension StakingMinimumAmountValidator: SendAmountValidator {
    func validate(amount: Decimal) throws {
        if let minAmount = minimumAmount.withLock({ $0 }), amount < minAmount {

...
class CommonUnstakingStepsManager {
    ...
    private func getNextStep() -> SendStep? {
        ...
        case .amount:
            return summaryStep
        case .destination, .fee, .validators, .summary, .finish, .onramp, .newAmount, .newDestination, .newSummary:
    ...
    private func next(step: SendStep) {
        ...
        case .amount, .destination, .fee, .validators, .onramp, .newAmount, .newDestination, .newSummary:
            assertionFailure("There is no next step")
        }
    }

...
final class MarketsTokenDetailsCoordinator: CoordinatorObject {
    ...
    /// AddFunds presented modally (full-screen cover) from a Markets token.
...
extension MarketsTokenDetailsCoordinator: MarketsPortfolioContainerRoutable {
    ...
    /// The token list ("довыбор") stays a floating sheet; once a token is picked it's dismissed and AddFunds is shown modally.
    @MainActor

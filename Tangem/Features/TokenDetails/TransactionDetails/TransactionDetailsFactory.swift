...
struct TransactionDetailsFactory {
    ...
    private func content(
        ...
        case nil:
            return sendReceiveContent(for: transaction, record: record, context: context)
        }
    }
    ...
    // HARDCODE: banner titles/subtitles are English literals until the localization keys are added in Lokalise.
    ...
    private func onrampContent(
        ...
        let received = OnrampTransactionDetailsViewData.ReceivedLeg(
            destinationName: context.receiverName,
    ...
    private func sendReceiveContent(
        ...
        if transaction.isOutgoing {
            return .send(SendTransactionDetailsViewData(
                tokens: tokens,
                recipient: counterparty(for: transaction, label: Localization.sendRecipient),
                info: networkFeeInfo(from: record)
            ))
        } else {
            return .receive(ReceiveTransactionDetailsViewData(
                tokens: tokens,
                sender: counterparty(for: transaction, label: Localization.commonFrom)
            ))
        }
    }
    ...
    private func provider(_ provider: ExpressProvider?, fallbackId: ExpressProvider.Id, externalURL: URL?, openURL: @escaping (URL) -> Void) -> TransactionDetailsProvider {
        TransactionDetailsProvider(
            name: provider?.name ?? fallbackId,
            iconURL: provider?.imageURL,
            // Tappable (with the trailing arrow) only when there's an external link to open.
    ...
    private func networkFeeInfo(from record: TransactionRecord?) -> TransactionDetailsInfoSectionViewData? {
        ...
        return .init(rows: [.init(title: Localization.commonNetworkFeeTitle, content: .text(fee))])
    }
    ...
    private func counterparty(for transaction: TransactionViewModel, label: String) -> TransactionDetailsAddressViewData? {
        ...
        case .unresolved(let short, let fullAddress, let blockiesImage):
            return .init(
                label: label,
                actor: .address(short: short, blockiesImage: blockiesImage),

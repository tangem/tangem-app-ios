...
extension PendingExpressTxStatusBottomSheetViewModel {
    func buildShareText() -> String {
        ...
        lines.append("")
        lines.append("By \(pendingTransaction.provider.name) \(pendingTransaction.provider.type.rawValue.uppercased())")
        lines.append("ID: \(pendingTransaction.externalTxId ?? pendingTransaction.expressTransactionId)")

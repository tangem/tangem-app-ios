...
class BitcoinTests: XCTestCase {
    ...
    private func testTransaction(_ transaction: BlockchainSdk.Transaction, signatures: [Data], txBuilder: BitcoinTransactionBuilder, sortType: TransactionDataSortType, expectedHashes: [Data], expectedSignedTransaction: Data) throws {
        let buildToSignResult = try txBuilder.buildForSign(transaction: transaction, sequence: 4294967290, sortType: sortType)
        sizeTester.testTxSizes(buildToSignResult)
        let signedTx = try txBuilder.buildForSend(transaction: transaction, signatures: signatures, sequence: 4294967290, sortType: sortType)

...
struct UTXOTransactionSerializerTests {
    ...
    func legacy() throws {
        ...
        // when
        let hashes = try builder.preImageHashes(transaction: preImage)
        let encoded = try builder.compile(transaction: preImage, signatures: signatures)
    ...
    func legacy_withMemo() throws {
        ...
        let preImageNoMemo = PreImageTransaction(
            inputs: [
                .init(output: utxo, script: sourceScript),
            ],
            outputs: [
        ...
        let preImageWithMemo = PreImageTransaction(
            inputs: [
                .init(output: utxo, script: sourceScript),
            ],
            outputs: [
            ...
            fee: .zero, // Not used
            opReturn: (txWithMemo.params as? BitcoinTransactionParams)?.memo
        )
        ...
        // when
        let hashesNoMemo = try builder.preImageHashes(transaction: preImageNoMemo)
        let hashesWithMemo = try builder.preImageHashes(transaction: preImageWithMemo)
        let encoded = try builder.compile(transaction: preImageWithMemo, signatures: signatures)
